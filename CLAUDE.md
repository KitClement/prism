...
- **`src/lib/`** (pure, no React)
  ...
  - `sampling.js` — the **stage** model (`mkStage`/`toStages`/`migratePipeline`+`rekeyStats`/
    ...
    **declared outcomes**).
  - `share.js` — URL sharing (Task C/D): `encodeConfig`/`decodeConfig`/`checkHiddenPassword`/
    `shareURL`, lz-string compression, and the XOR password-veil helpers (not crypto-grade).
    **Size is the design constraint** — see "URL sharing" below for the compact v2 format.
...
- **URL sharing** (`lib/share.js`, the one new dependency **lz-string**): `encodeConfig`
  serializes the minimal config (`pipeline`, `sampleSize`, `runMode`/`stopRule`, `codeLang`)
  → a `?s=` blob; a one-time mount `useEffect` in `App.jsx` imports it, runs `migratePipeline`
  + `rekeyStats`/`rekeyStopRule`, then cleans the URL. `trackedStats` are deliberately **not**
  serialized (they're regenerated by re-running the sampler) — which is also what lets the
  encoder drop ids, since nothing outside the pipeline references them.
- **The compact blob (format v2)** — the payload is minified **hard** before compressing,
  because the compressor can only mask verbosity, not remove it. This is what keeps a typical
  link ~110 chars instead of ~530, and a hidden one ~110 instead of ~2200. The first char is a
  marker we author (`P` plain / `H` hidden), so the format is never guessed at:
  - **Tuple form, no ids, no defaults.** A stage's identity in the blob is its **position**, so
    a branch condition and `stopRule.stageId` store the upstream stage's **index**; decode mints
    fresh uids and rewires them (so `migratePipeline`/`rekey*` are identity no-ops on a shared
    link). Colors store a `COLORS` index (literal hex only when off-palette), spinner `pct`
    rounds to 4dp (`sampleSpinner` normalizes by the total, so the residual divides out), and
    mixer **balls run-length group** by `(label,color)` — a ball order that isn't already
    contiguous is kept exactly via a base36 order string, so ball count is nearly free while
    a CSV-pasted (interleaved) mixer still reopens ball-for-ball identical.
  - **v2 is a clean break**: v1 blobs are *detected* (`{stale:true}`) and reported, not decoded
    — a stale link says so instead of silently opening a blank sampler.
  - Everything in the URL must live in lz-string's URI-safe alphabet. Two traps, both fixed and
    regression-worthy: the password verifier is folded through `toAlpha` (raw `deriveKey` output
    spans ASCII 33-126 — a `#` would truncate the whole link), and decode restores `+` from the
    space a query-string parse turns it into.
- **Hidden samplers** (Task D): a hidden link **opens and runs for anyone — no password
  ...
  (`checkHiddenPassword`, a truncated prefix — the compare is length-adaptive), never the
  plaintext. **Not** crypto-grade. **Compress first, veil second** (`xorAlpha` shifts within the
  URI-safe alphabet): v1 XOR'd *before* compressing, handing the compressor pseudo-random bytes
  it couldn't pack and then paying base64 +33% plus 6 bytes per JSON-escaped control char — which
  is why v1 hidden links ran ~20× longer than needed. A hidden sampler renders opaque
  placeholders + a "Reveal" button until the password is entered. Browser dialogs are
